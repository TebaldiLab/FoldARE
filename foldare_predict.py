...
def predict_structure(
    ...
    if ptool == "eternafold":
        ...
        max_str_in_out=1 # conformers in output for best (final) RNAstr --> 1 = only best; for more set it to > 1
        kw = filter_kwargs(
            RNAStructure,
            {
                "seq_file": args.sequence,
...
def main():
    ...
    ############ USE SPACE BELOW to edit all db output file in a similar manner? (>head\nseq\npred\t(MFE or MEA) --# I would remove energy..
    if predictor_tool.lower()=="linearfold":
        LF_compatible=open(pred_out).read().replace("Unrecognized sequence: ","")
        Lf2=open(pred_out,"w");Lf2.write(LF_compatible);Lf2.close()
    elif predictor_tool.lower()=="eternafold":
        temp=open(pred_out).read().replace("Predicting using MEA estimator.\n","").replace(">structure\n","")
        try:
            EE=temp.split("\n"); head=">"+base_name; seq_f=EE[1]; ss_f=EE[2]; EF_compatible=head+"\n"+seq_f+"\n"+ss_f
        except:
            pass
        Ef2=open(pred_out,"w"); Ef2.write(EF_compatible); Ef2.close()        
    os.system(ps_exec+" "+pred_out+" -c "+pred_out.replace(".db","")+".ps")
